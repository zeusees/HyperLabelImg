...
#include <CLM/LandmarkDetectorFunc.h>
...
bool LandmarkDetector::DetectLandmarksInVideo(const cv::Mat_<uchar> &grayscale_image, const cv::Mat_<float> &depth_image, CLNF& clnf_model, FaceModelParameters& params)
{
	...
	if ((!clnf_model.tracking_initialised && (clnf_model.failures_in_a_row + 1) % (params.reinit_video_every * 3) == 0)
		|| (clnf_model.tracking_initialised && !clnf_model.detection_success && params.reinit_video_every > 0 && clnf_model.failures_in_a_row % params.reinit_video_every == 0))
	{
		...
		clock_t t1 = clock(); 
		cv::Mat imgBGR;
		cvtColor(grayscale_image,imgBGR,CV_GRAY2BGR);
		face_detection_success = ncnn_detect(imgBGR, bounding_box);
		clock_t t2 = clock();
#ifdef DEBUG_STRINGMY
		printf("Invideo SingleYU cost %d BB %d \n",t2-t1, face_detection_success);
#endif
		...
		if(face_detection_success)
		{
			...
			clock_t t4 = clock();
#ifdef DEBUG_STRINGMY
			printf("clnf_model cost %d B %d \n", t4 - t3, landmark_detection_success);
#endif
