...
void PDM::ComputeRigidJacobian(const Mat_<float>& p_local, const Vec6d& params_global, cv::Mat_<float> &Jacob, const Mat_<float> W, cv::Mat_<float> &Jacob_t_w)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		X = ((float*)shape_3D.data)[i];
		Y = ((float*)shape_3D.data)[i + n];
		Z = ((float*)shape_3D.data)[i + 2 * n];
		...
		// scaling term
		((float*)Jacob.data)[indexJ] = (X  * r11 + Y * r12 + Z * r13);
		((float*)Jacob.data)[indexJ + jumpJx] = (X  * r21 + Y * r22 + Z * r23);
		indexJ++;

		// rotation terms
		((float*)Jacob.data)[indexJ] = (s * (Y * r13 - Z * r12));
		((float*)Jacob.data)[indexJ + jumpJx] = (s * (Y * r23 - Z * r22));
		indexJ++;


		((float*)Jacob.data)[indexJ] = (-s * (X * r13 - Z * r11));
		((float*)Jacob.data)[indexJ + jumpJx] = (-s * (X * r23 - Z * r21));
		indexJ++;

		((float*)Jacob.data)[indexJ] = (s * (X * r12 - Y * r11));
		((float*)Jacob.data)[indexJ + jumpJx] = (s * (X * r22 - Y * r21));
		indexJ++;

		// translation terms
		((float*)Jacob.data)[indexJ] = 1.0f;
		((float*)Jacob.data)[indexJ + jumpJx] = 0.0f;
		indexJ++;

		((float*)Jacob.data)[indexJ] = 0.0f;
		((float*)Jacob.data)[indexJ + jumpJx] = 1.0f;
		indexJ++;
	...
	for (int i = 0; i < n; i++)
	{
		float w_x = W.at<float>(i, i);//((float*)W.data)[i*W.cols+i];
		float w_y = W.at<float>(i + n, i + n);// ((float*)W.data)[(i+n)*W.cols+i+n];

		for (int j = 0; j < Jacob.cols; ++j)
		{
			((float*)Jacob_w.data)[j] = ((float*)Jacob.data)[j] * w_x;
			((float*)Jacob_w.data)[j + jumpJx] = ((float*)Jacob.data)[j + jumpJx] * w_y;
		}
	}
...
void PDM::ComputeJacobian(const Mat_<float>& params_local, const Vec6d& params_global, Mat_<float> &Jacobian, const Mat_<float> W, cv::Mat_<float> &Jacob_t_w)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		// scaling term
		((float*)(Jacobian.data))[idexJ] = (X  * r11 + Y * r12 + Z * r13);
		((float*)(Jacobian.data))[idexJ + jumpJ] = (X  * r21 + Y * r22 + Z * r23);
		idexJ++;

		// rotation terms
		((float*)(Jacobian.data))[idexJ] = (s * (Y * r13 - Z * r12));
		((float*)(Jacobian.data))[idexJ + jumpJ] = (s * (Y * r23 - Z * r22));
		idexJ++;

		((float*)(Jacobian.data))[idexJ] = (-s * (X * r13 - Z * r11));
		((float*)(Jacobian.data))[idexJ + jumpJ] = (-s * (X * r23 - Z * r21));
		idexJ++;

		((float*)(Jacobian.data))[idexJ] = (s * (X * r12 - Y * r11));
		((float*)(Jacobian.data))[idexJ + jumpJ] = (s * (X * r22 - Y * r21));
		idexJ++;

		// translation terms
		((float*)(Jacobian.data))[idexJ] = 1.0f;
		((float*)(Jacobian.data))[idexJ + jumpJ] = 0.0f;
		idexJ++;


		((float*)(Jacobian.data))[idexJ] = 0.0f;
		((float*)(Jacobian.data))[idexJ + jumpJ] = 1.0f;
		idexJ++;

		for (int j = 0; j < m; j++)
		{
			Vx = ((double*)princ_comp.data)[jumpV];
			Vy = ((double*)princ_comp.data)[jumpV + jumpVx];
			Vz = ((double*)princ_comp.data)[jumpV + jumpVy];

			// How much the change of the non-rigid parameters (when object is rotated) affect 2D motion
			((float*)(Jacobian.data))[idexJ] = (float)(s*(r11*(Vx)+r12*(Vy)+r13*(Vz)));
			((float*)(Jacobian.data))[idexJ + jumpJ] = (float)(s*(r21*(Vx)+r22*(Vy)+r23*(Vz)));
	...
	if (cv::trace(W)[0] != W.rows)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			{
				((float*)Jacob_w.data)[j] = ((float*)Jacobian.data)[j] * w_x;
				((float*)Jacob_w.data)[j + jumpJ] = ((float*)Jacobian.data)[j + jumpJ] * w_y;
			}
		}
	}
	Jacob_t_w = Jacob_w.t();
...
//===========================================================================
// Get the 2D shape (in image space) from global and local parameters
void PDM::CalcShape2DMY(cv::Mat_<double>& out_shape, const cv::Mat_<double>& params_local, const cv::Vec6d& params_global) const
{
	int n = this->NumberOfPoints();
	...
	for (int i = 0; i < n; i++)
	{
		...
		out_shape.at<double>(i + 2 * n, 0) = s * (currRot(2, 0) * Shape_3D.at<double>(i, 0) + currRot(2, 1) * Shape_3D.at<double>(i + n, 0) + currRot(2, 2) * Shape_3D.at<double>(i + n * 2, 0));
		//double t = 	out_shape.at<double>(i+n,0) ;
		//out_shape.at<double>(i+n*2,0) = s *Shape_3D.at<double>(i+n*2,0);
	}
}
