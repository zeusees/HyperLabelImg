...
void CLNF::CLM_CalcParams(cv::Vec6d& out_params_global, cv::Mat_<double>& out_params_local,
	const cv::Mat_<double>& landmark_locations)
{
	out_params_local = cv::Mat_<double>(pdm.NumberOfModes(), 1, 0.0);
	pdm.CalcParams(out_params_global, out_params_local, landmark_locations);
}
//=============================================================================
//=============================================================================
...
void CLNF::Read_CLNF(string clnf_location)
{
	...
	int index = clnf_location.rfind('/');
	string root(clnf_location, 0, index + 1);
...
void CLNF::Read(string main_location)
{
	...
	int index = main_location.rfind('/');
	string root(main_location, 0, index + 1);
	// The main file contains the references to other files
	while (!locations.eof())
	{ 
		getline(locations, line);
...
void savePtsFile(const char * IdName, vector<cv::Point_<double>>& points)
{
...
bool CLNF::Fit(const cv::Mat_<uchar>& im, const cv::Mat_<float>& depthImg, const std::vector<int>& window_sizes, const FaceModelParameters& parameters)
{
	...
	for(int scale = 0; scale < num_scales; scale++)
	{
		...
		//DEBUG_STRING("reponse %d ..\n", t2 - t1);
		// Get the view used by patch experts
		int view_id = patch_experts.GetViewIdx(params_global, scale);
#ifdef LOG_WRITE
		char szBuf[512];
		sprintf(szBuf,"./img_log/1_%d_response.bmp", scale, gMatResponse);
		imwrite(szBuf,gMatResponse);
#endif LOG_WRITE
	...
	int64 timNow = cv::getTickCount();

	

	//37,44
#ifdef LOG_WRITE
	sprintf(szBuf,"./test_log/%lld.bmp", timNow);
	cv::imwrite(szBuf,tmpMat);

	sprintf(szBuf, "./test_log/%lld.pts", timNow);
	savePtsFile(szBuf, pts);
#endif

	return true;
}

void CLNF::PreComputeMeanshift_KDE()
{

	cv::Mat_<float> kde_resp;
	float step_size = 0.1;
	
	for (int resp_size = 7; resp_size <= 11; resp_size += 2)
	{
		float a = -0.222222224;
		char szBuf[512];
		sprintf(szBuf, "model/patch_cal_result/kde_resp_%d.bin", resp_size);
		string filename = std::string(g_rootpath) + szBuf;
		if (!ISFileExist((char*)filename.c_str()))
		{
			//
			kde_resp = cv::Mat_<float>((int)((resp_size / step_size)*(resp_size / step_size)),
				resp_size * resp_size);
			...
			fwrite(kde_resp.data, 1, kde_resp.step[0] * kde_resp.rows, pfile);
			fclose(pfile);
		}
		else
		{
			FILE * fRead = fopen(filename.c_str(), "rb");
			cv::Mat_<float> Sigma_f = cv::Mat::ones((resp_size / step_size)*
				(resp_size / step_size), resp_size*resp_size, CV_32F);
			int rLen = fread(Sigma_f.data, 1, Sigma_f.step[0] * Sigma_f.rows, fRead);
			fclose(fRead);

			kde_resp_precalc[resp_size] = Sigma_f;
			kde_resp = Sigma_f;
		}
...
void CLNF::NonVectorisedMeanShift_precalc_kde(cv::Mat_<float>& out_mean_shifts, const vector<cv::Mat_<float> >& patch_expert_responses, const cv::Mat_<float> &dxs, const cv::Mat_<float> &dys, int resp_size, float a, int scale, int view_id, map<int, cv::Mat_<float> >& kde_resp_precalc)
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		{
			((float*)out_mean_shifts.data)[i*out_mean_shifts.cols] = 0;
			((float*)out_mean_shifts.data)[(i + n)*out_mean_shifts.cols] = 0;
			continue;
		}


		float dx = ((float*)dxs.data)[i];
		float dy = ((float*)dys.data)[i];
		...
		int stepW = kde_resp.cols*idx;
		float mx = 0.0;
		float my = 0.0;
		float sum = 0.0;

		// Iterate over the patch responses here
		cv::Mat_<float> p = patch_expert_responses[i];
		...
		for (int ii = 0; ii < resp_size; ++ii)
		{
			...
			for (int jj = 0; jj < resp_size; ++jj)
			{
				...
				float v = (((float*)p.data)[step + jj]) * (((float*)kde_resp.data)[stepW + step + jj]);
		...
		((float*)out_mean_shifts.data)[i*out_mean_shifts.cols] = msx;
		((float*)out_mean_shifts.data)[(i + n)*out_mean_shifts.cols] = msy;
...
#ifdef LOG_WRITE
		char szBuf[512];
		sprintf(szBuf,"./img_log/2_%d_%d_%d NU_RLMS.bmp",gIter,rigid,iter);
		for (int mi = 0; mi < 16; mi++)
		{
			cv::line(gGrayMat,cv::Point(current_shape.at<double>(mi, 0), current_shape.at<double>(mi+68, 0)),
				cv::Point(current_shape.at<double>(mi+1, 0), current_shape.at<double>(mi + 68+1, 0)),cv::Scalar(255),1, cv::LINE_AA);
			//cv::circle(gGrayMat, cv::Point(current_shape.at<double>(mi, 0), current_shape.at<double>(mi + current_shape.rows/2, 0)),
			//	3, cv::Scalar(255, 0, 0), 3, cv::LINE_AA);
			char szBuf[512];
			sprintf(szBuf,"%d",mi);
			//cv::putText(gGrayMat, szBuf, cv::Point(current_shape.at<double>(mi, 0), current_shape.at<double>(mi + current_shape.rows / 2, 0)),
			//	0, 0.2, cv::Scalar(255), 1, cv::LINE_AA);
		}
		cv::imwrite(szBuf, gGrayMat);
#endif
		if(iter > 0)
		{
