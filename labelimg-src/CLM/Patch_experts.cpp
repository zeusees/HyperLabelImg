							 ...
							 const PDM& pdm, const cv::Vec6d& params_global, const cv::Mat_<double>& params_local, int window_size, int scale)
{
	...
	gMatResponse = grayscale_image.clone();
	...
	cv::Mat_<double> Tlandmark_locations;
	pdm.CalcShape2D(Tlandmark_locations, params_local, params_global);
...
void Patch_experts::DoCompute()
{
	...
	for (int view_id = 0; view_id < 2; ++view_id)
	{
		...
		{
			for (int lmark = 0; lmark <= 67; ++lmark)
			{
				for (int window_size = 7; window_size <= 11; window_size += 2)
				{
					std::vector<cv::Mat_<float> > sigma_components;
					for (int w_size = this->sigma_components.size() - 1; w_size >= 0; --w_size)
					{
						if (window_size*window_size == this->sigma_components[w_size][scale].rows)
						{
							sigma_components = this->sigma_components[w_size];
							break;
						}
					...
					{
						// Precompute sigmas if they are not computed yet
						ccnf_expert_intensity[scale][view_id][lmark].ComputeFromFile(sigma_components,
							scale, view_id, lmark, window_size);
...
void Patch_experts::Read_CCNF_patch_experts(string patchesFileLocation, std::vector<cv::Vec3d>& centers, 
