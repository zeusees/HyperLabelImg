...
void Draw(cv::Mat img, const cv::Mat_<double>& shape2D)
{
	...
	{		
		...
		char szBuf[512];
		sprintf(szBuf,"%d",i);
		cv::putText(img,szBuf, featurePoint,0,0.3,cv::Scalar(255,0,0),1, CV_AA);
	}
...
bool DetectFacesHOG(vector<cv::Rect_<double> >& o_regions, const cv::Mat_<uchar>& intensity, void* detector, std::vector<double>& o_confidences)
{
	...
	//(*(dlib::frontal_face_detector*)detector)(cv_grayscale, face_detections, -0.2);
...
bool DetectSingleFaceHOG(cv::Rect_<double>& o_region, const cv::Mat_<uchar>& intensity_img, void *  detector, double& confidence, cv::Point preference)
{
	// The tracker can return multiple faces
	vector<cv::Rect_<double> > face_detections;
	vector<double> confidences;

	bool detect_success = LandmarkDetector::DetectFacesHOG(face_detections, intensity_img, 
